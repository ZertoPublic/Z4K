...
USAGE_MSG = ("Usage: '!!zswagger <filename>!!' or '!!zswagger-http <url>!!'. "
...
ERROR_TEMPLATE = string.Template("!! ZSWAGGER ERROR: $error !!")

# Used for JS. Runs locally on end-user. RFI / LFI not possible, no security risk.
# Restrict to local file.
TOKEN = re.compile(r"!!zswagger(?: (?P<path>[^\\/\s><&:]+))?!!")

# HTTP(S) variant
TOKEN_HTTP = re.compile(r"!!zswagger-http(?: (?P<path>https?://[^\s]+))?!!")
...
class ZertoSwaggerPlugin(mkdocs.plugins.BasePlugin):
